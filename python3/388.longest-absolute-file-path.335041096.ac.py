...
class Solution:
    def lengthLongestPath(self, input: str) -> int:
        lengths = []
        cumulative_length = max_length = 0
        files = input.split('\n')
        for f in files:
            file = False
            depth = 0
            for c in f:
                if c == '\t':
                    ...
                    file = True
                    break
            folder_length = len(f) - depth + (1 if depth else 0)
            while depth < len(lengths):
                cumulative_length -= lengths.pop()
            cumulative_length += folder_length
            lengths.append(folder_length)
            if file:
                max_length = max(max_length, cumulative_length)
        return max_length
# @lc code=end
