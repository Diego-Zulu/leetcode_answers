...
class Solution:
    def calPoints(self, ops):
        ...
        for o in ops:
            if o == '+':
                current_score = valid_rounds[-1] if len(valid_rounds) > 0 else 0
                current_score += valid_rounds[-2] if len(valid_rounds) > 1 else 0
                
            elif o == 'D':
                current_score = 2 * valid_rounds[-1] if len(valid_rounds) > 0 else 0
                
            elif o == 'C':
                current_score = -1 * valid_rounds.pop()
            ...
            score += current_score
            if o != 'C':
                valid_rounds.append(current_score)
