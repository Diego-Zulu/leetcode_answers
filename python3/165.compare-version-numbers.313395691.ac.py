...
class Solution:
    def compareVersion(self, version1: str, version2: str) -> int:
        separate1 = version1.split('.')
        separate2 = version2.split('.')
        while len(separate1) > len(separate2):
            separate2.append('0')
        while len(separate1) < len(separate2):
            separate1.append('0')
        for first, second in zip(separate1, separate2):
            first_cast = int(first)
            second_cast = int(second)
            if first_cast > second_cast:
                return 1
            elif first_cast < second_cast:
                return -1
        return 0
# @lc code=end
