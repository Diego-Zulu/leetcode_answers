...
class Solution {
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        HashMap<String, Integer> domainsAndCounts = new HashMap<>();

        for (int i=0; i<cpdomains.length; i++) {

            String[] numberAndDomainNames = cpdomains[i].split(" ");
            String currentWord = numberAndDomainNames[1];
            int number = Integer.parseInt(numberAndDomainNames[0]);
            StringBuilder currentSubDomain = new StringBuilder();

            for (int j=currentWord.length() - 1; j >= 0; j--) {

                if (currentWord.charAt(j) == '.') {
                    addSubdomainCountToMap(domainsAndCounts, currentSubDomain, number);
                }
                currentSubDomain.append(currentWord.charAt(j));
            }
            addSubdomainCountToMap(domainsAndCounts, currentSubDomain, number);
        }

        return domainsAndCountsList(domainsAndCounts);
    }

    private void addSubdomainCountToMap(HashMap<String, Integer> domainsAndCounts, StringBuilder currentSubDomain, int number) {
        String subDomain = currentSubDomain.reverse().toString();
        currentSubDomain.reverse();
        int newCounts = domainsAndCounts.getOrDefault(subDomain, 0) + number;
        domainsAndCounts.put(subDomain, newCounts);
    }

    private List<String> domainsAndCountsList(HashMap<String, Integer> domainsAndCounts) {

        List<String> list = new LinkedList<>();
        for (Map.Entry<String, Integer> e : domainsAndCounts.entrySet()) {
            StringBuilder element = new StringBuilder();
            element.append(e.getValue());
            element.append(" ");
            element.append(e.getKey());
