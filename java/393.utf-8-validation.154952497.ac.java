...
class Solution {
    public boolean validUtf8(int[] data) {
        ...
        for (int i=0; i<data.length; i++) {
            ...
            if (data[i] > 255) {
                ...
                int masked = (data[i] & 63) ^ data[i];
                if (masked != 128) return false;
                bitsLeft--;
            } else {
                int countedBits = count1Bits(data[i]);
                if (countedBits > 4 || countedBits == 1) return false;
                bitsLeft = Math.max(countedBits - 1, 0);
            }
        }
    ...
    private int count1Bits(int numb) {
        
        int mask = 128;
        int bits = 0;
        while (mask > 0 && bits <= 4) {
            if ((numb & mask) == 0) return bits;
            bits++;
            mask = mask>>1;
        }
        return bits;
