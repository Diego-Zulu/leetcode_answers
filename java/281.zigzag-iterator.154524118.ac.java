/*
 * @lc app=leetcode id=281 lang=java
 *
 * [281] Zigzag Iterator
 *
 * https://leetcode.com/problems/zigzag-iterator/description/
 *
 * algorithms
 * Medium (57.87%)
 * Likes:    352
 * Dislikes: 20
 * Total Accepted:    62.2K
 * Total Submissions: 107.4K
 * Testcase Example:  '[1,2]\n[3,4,5,6]'
 *
 * Given two 1d vectors, implement an iterator to return their elements
 * alternately.
 * 
 * 
 * 
 * Example:
 * 
 * 
 * Input:
 * v1 = [1,2]
 * v2 = [3,4,5,6] 
 * Output: [1,3,2,4,5,6]
 * Explanation: By calling next repeatedly until hasNext returns false, the
 * order of elements returned by next should be: [1,3,2,4,5,6].
 * 
 * 
 * 
 * Follow up:
 * 
 * What if you are given k 1d vectors? How well can your code be extended to
 * such cases?
 * 
 * Clarification for the follow up question:
 * The "Zigzag" order is not clearly defined and is ambiguous for k > 2 cases.
 * If "Zigzag" does not look right to you, replace "Zigzag" with "Cyclic". For
 * example:
 * 
 * 
 * Input:
 * [1,2,3]
 * [4,5,6,7]
 * [8,9]
 * 
 * Output: [1,4,8,2,5,9,3,6,7].
 * 
 * 
 */

// @lc code=start
public class ZigzagIterator {
    
    List<Iterator<Integer>> listsToIterate;
    Iterator<Iterator<Integer>> listsIterator;

    public ZigzagIterator(List<Integer> v1, List<Integer> v2) {
        listsToIterate = new LinkedList<>();
        if (v1.size() > 0) {
            listsToIterate.add(v1.iterator());
        }
        if (v2.size() > 0) {
            listsToIterate.add(v2.iterator());
        }
        listsIterator = listsToIterate.iterator();
    }

    public int next() { 
        
        Iterator<Integer> nextIt = getNextUnderlyingListIterator();
        Integer nextElement = nextIt.next();
        
        if (!nextIt.hasNext()) {
            listsIterator.remove();
        }
        
        return nextElement;
    }
    
    private Iterator<Integer> getNextUnderlyingListIterator() {
        if (!listsIterator.hasNext()) {
            listsIterator = listsToIterate.iterator();
        }
        return listsIterator.next();
    }

    public boolean hasNext() {
        
        return listsToIterate.size() > 0;
        
        
    }
}

/**
 * Your ZigzagIterator object will be instantiated and called as such:
 * ZigzagIterator i = new ZigzagIterator(v1, v2);
 * while (i.hasNext()) v[f()] = i.next();
 */
// @lc code=end
